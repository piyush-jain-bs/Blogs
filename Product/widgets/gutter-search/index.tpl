...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function(){
    new OverText($('product_search_field'), {
      poll: true,
      pollInterval: 500,
      positionOptions: {
...
<?php $searchUrl = $this->url(array('user_id' => $this->owner->getIdentity()), 'product_view', true); ?>

<form id='filter_form' class="product_search_form" method='GET' action="<?php echo $this->escape($searchUrl) ?>">
  <input type='text' class='text suggested' name='search' id='product_search_field' size='20' maxlength='100' alt='<?php echo $this->translate('Search Products') ?>' value="<?php if( $this->search ) echo $this->escape($this->search); ?>" />
